...
По нажатию **одного** хоткея (по умолчанию `<Super>w` — см. секцию «Горячие клавиши») расширение само выбирает сторону прижатия:
...
- `toggle-snap` — переключатель. Дефолт: `<Super>w`.
